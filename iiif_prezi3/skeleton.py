...
#   timestamp: 2025-10-21T21:13:36+00:00
...
class NavDate(RootModel):
    root: datetime


class Rights(RootModel):
    root: AnyUrl = Field(
        ...,
...
class ViewingDirection(RootModel):
    root: Union[
...
class BCP47(RootModel):
    root: Union[constr(pattern=r'^[a-zA-Z-][a-zA-Z-]*$'), constr(pattern=r'^none$')] = (
        Field(..., title='BCP47')
    )


class Dimension(RootModel):
    root: PositiveInt = Field(..., title='dimension')


class Duration(RootModel):
    root: PositiveFloat = Field(..., title='duration')
...
class LinkedResources(RootModel):
...
class PartOf(RootModel):
    root: List[Class]


class SeeAlso(RootModel):
    root: LinkedResources


class AbstractIIIFResource(RootModel):
...
class AnnotationCollectionRef(RootModel):
    root: Union[str, AnnotationCollectionRefExtended]


class AnnotationPageRef(RootModel):
